...
#       @author       :Ling hao
#       @qq           :119642282@qq.com
#       @file         :/home/lhw4d4/project/linghao\client.c
#       @date         :2015-08-31 09:32
#       @algorithm    :
=========================================================================*/
#include <sys/types.h>
...
#define HOST_ADDRESS "192.168.42.10"
#define PLC_ADDRESS "192.168.1.121"
...
void dbinit()
{
	...
	if(rc!=SQLITE_OK)
	{
		fprintf(stderr,"connot open database:%s\n",sqlite3_errmsg(db));
		sqlite3_close(db);
		pthread_exit((void *)1);
	}
	else
...
int real_time_data()
{
	...
	char c;
	char d;
	char real[15]="real_time_data";
	fp2=fopen(DEV_CONF,"r");
	if(fp2==NULL)
		printf("open file error\n");
	while((fgets(temp,50,fp2))!=NULL)
	{
		if(strncmp(temp,"real_time_data",14)==0)
		{
			p=strchr(temp,'=');
	...
	if(c!=d)
	{
		sprintf(temp,"real_time_data=%c",c);
		addoraltconfig(DEV_CONF,"read_time_data",temp);
		send_signal_usr2();
	}
	return 1;
}
...
//	printf("recv ok**************\n%s\n",read_buf);
	...
	{
		perror("local_first_login error");
		exit(1);
	}
	while(get_line(fd,read_buf,4096)>1);
	//write_buf="LGIN 002\r\n";
...
//	printf("recv ok***************\n%s\n",read_buf);
	...
	{
		perror("local_LOGI1  ERROR\r\n");
		exit(1);
	}
	addoraltconfig(DEV_CONF,"init","init=1");
	get_line(fd,read_buf,4096);
//	printf("recv ok**************\n%s\n",read_buf);
...
//					printf("6\n");
			...
			if(flag)
			{
				...
				write(socketfd,sendbuf,strlen(sendbuf));
				write(socketfd,sdata.data,sdata.length);
